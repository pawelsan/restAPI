                ...
                if (!movie) {
                    ...
                    comment.save().then(result => {
                        res.status(201).render('post-status', {
                            message: `Created comment: "${result.content}". With relation to ${movie.title} (movie ID: ${result.movie}). Comment ID ${result._id}. You can request it with "GET" from: https://movieapi2020.herokuapp.com/comments/${result._id}`
                        })
                    }).catch(err => {
